...
Суммарный объем трафика за сутки равен примерно 30 Pb, найдем объекм трафика в секунду:

30000 Tb / (24 * 60 * 60) = 350 Gb/sec

С учетом времени суток, пиоквая нагрузка может быть около 700 Gb/sec.

Для того, чтобы посчитать объем необходимой памяти также возьмем горизонт пять лет. Тогда посчитаем, какое колличество часов видео будет загружено за это время: 100(часов видео загружаемые в минуту) * 60 * 24 * 365 * 5 = 262 800 000 часов. Для хранения одного часа видео необходимо 2.25 Gb, следовательно все видео будут весить 262 800 000 *  2.25 Gb = 591 300 000 Gb = 591 Pb. Тк для надежности каждое видео будет хранится в двух экземплярах, то умножим это число на два, соотвественно получаем 591 Pb * 2 = 1182 Pb.



В результате получаем, что для хранения видео, нам необходимо примерно 1200 Pb.
...
Так как наиболее просматриваемыми мы считаем 1% от всех видео, то для их хранения нам потребуется примерно 11 Pb, при среднем объеме диска в 2 tb, получим, что нам необходимо **5500 дисков SSD объема по 2tb**.

**Если в один сервер ставить по 10 дисков, то получим, что хранения данных в медленном слое нам потребуется примерно 550 серверов.**

В пункте 5.3 было найдено, что нам необходима отдавать 350 GB в секунду. Средняя скорость чтения одного SSD диска равна примерно 2000 Mb в секунду, поэтому для закрытия потребности в чтении видео нам необходимо примерно 175 SSD дисков, то есть по скорости чтения мы укладываемся.

Для хранения всех видео за 5 лет нам недобхимо  1182 Pb(см. п. 3), тк мы будем использовать шардирование, где есть один мастер и два слейва, то нам необходимо будет примерно 2000 Pb памяти. То есть для хранения всех видео с учетом реплик нам будет необходимо **400 000 HDD дисков объемом 5 терабайт**.

**Если в один сервер ставить по 10 дисков, то получим, что хранения данных в медленном слое нам потребуется примерно 40 000 серверов.**



<h2>8. Хостинг </h2>

Самым дешевым способом работы такого колличества серверов, как мне кажется, можно считать строительство нескольких собственных датацентров в разных частях России.



<h2>9. Балансировка </h2>

На входе стоит DNS балансировка с локальным DNS сервером.

Для балансировки будем использовать сеть Nginx'ов с использованием схемы L7. Тк он решает проблемы клиентов с медленным интернетом(60% трафика с телефонов). Помимо этого, тк весь трафик идет через https или http2, то в Nginx есть возможность быстро воспользоваться этим протоколом.
...
Помимо всего прочего необходимо будет настроить монтироринг за основными показателями системы и создать дежурства, в том числе ночные и выходные дни.