...
**Если в один сервер ставить по 20 дисков, то получим, что хранения данных в медленном слое нам потребуется примерно 30 серверов.**

В пункте 5.3 было найдено, что нам необходима отдавать 290 GB в секунду. **Поэтому сеть в сервере должна уметь отдавать 290/30 = 10 Gb/s.**
